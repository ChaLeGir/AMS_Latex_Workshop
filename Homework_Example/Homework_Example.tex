...
%I guess we need to define the color for ourselves!
\definecolor{coloradoRed}{rgb}{0.56,0.00,0.41}
...
Let $\mathbf{z}$ be distributed $MN(0,I_m)$ then we know that $\mathbf{z}^T \mathbf{z} = \sum_{i=1}^m \mathbf{z}_i^2$ is distributed $\chi^2(m)$.  -- a sum of iid $N(0,1)$ RVs. 
...
\begin{enumerate}
	\item For  the eigendecomposition $M= U D U^T$   show that the diagonal elements of $D$ must be either $0$ or $1$.\\ {\it \color{pink} This is provides an alternative proof that $tr(M) =k$. }
	\item Let  $U$ be an $n \times n$ orthonormal matrix and $\mathbf{z}$ be distributed $MN(0,I_n)$. Show that $U^T\mathbf{z}$ is also  distributed $MN(0,I_n)$.
	\item Let $\mathbf{z}$ be distributed $MN(0,I_n)$  show that  $\mathbf{z}^T M \mathbf{z} $ is  distributed $\chi^2(k)$.
\end{enumerate}
...
\begin{enumerate}
	...
	\item $U^T \mathbf{z} $ satisfies the definition that says $XXXXXX \qquad \square$
